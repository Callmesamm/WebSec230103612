...
class ProductsController extends Controller {
    ...
    public function buy(Request $request, Product $product){
        ...
        if ($user->credit < $product->price) {
            return redirect()->route('products.insufficient-funds', ['product' => $product->id]);
        }
    ...
    public function insufficientFunds(Request $request, Product $product)
    {
        if (!auth()->user()->hasRole('Customer')) {
            abort(403, 'Only customers can access this page.');
        }

        return view('products.insufficient_funds', compact('product'));
    }
}